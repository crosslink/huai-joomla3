				...
				<?php
					...
					switch ($item->params->get('target', $this->params->get('target')))
					{
						case 1:
							...
							echo '<h4 class="title" style="margin:0;padding:0;"><a href="'. $link .'" target="_blank" class="'. $menuclass .'" rel="nofollow">'.
								$this->escape($item->title) .'</a></h4>';
							break;

						case 2:
							// open in a popup window
							$attribs = 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width='.$this->escape($width).',height='.$this->escape($height).'';
							echo "<h4 class='title' style='margin:0;padding:0;'><a href=\"$link\" onclick=\"window.open(this.href, 'targetWindow', '".$attribs."'); return false;\">".
								$this->escape($item->title).'</a></h4>';
							break;
						case 3:
							// open in a modal window
							JHtml::_('behavior.modal', 'a.modal'); ?>
							<h4 class='title' style='margin:0;padding:0;'><a class="modal" href="<?php echo $link;?>"  rel="{handler: 'iframe', size: {x:<?php echo $this->escape($width);?>, y:<?php echo $this->escape($height);?>}}">
								<?php echo $this->escape($item->title). ' </a></h4>' ;
							break;

						default:
							// open in parent window
							echo '<h4 class="title" style="margin:0;padding:0;"><a href="'.  $link . '" class="'. $menuclass .'" rel="nofollow">'.
								$this->escape($item->title) . ' </a></h4>';
							break;
					}
				?>
